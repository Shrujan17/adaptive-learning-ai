...
async function saveSubsToFirestore(userId, subs) {
  try {
    await setDoc(doc(db, 'users', userId), { subjects: subs, updatedAt: new Date().toISOString() })
  } catch (e) { console.error('Firestore save error:', e) }
}

async function loadSubsFromFirestore(userId) {
  try {
...
async function callGemini(prompt, retries = 3) {
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (res.status === 429) {
      const retryAfter = d?.error?.details?.find(x => x.retryDelay)?.retryDelay
      const waitMs = retryAfter ? (parseFloat(retryAfter) * 1000) : (10000 * (attempt + 1))
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        setDbLoading(true)
        const saved = await loadSubsFromFirestore(firebaseUser.uid)
        setSubs(saved)
        setDbLoading(false)
  ...
  // Auto-save to Firestore when subs change
  useEffect(() => {
    if (user) saveSubsToFirestore(user.uid, subs)
  }, [subs, user])

  const notify = (m, err = false) => { setToast({ m, err }); setTimeout(() => setToast(null), 4000) }
  ...
  const login = async () => {
    try {
      ...
      console.error(e)
      notify('❌ Login failed: ' + e.message, true)
    }
  }
  ...
  const upload = async (file, name) => {
    setLoading(true)
    try {
      setMsg('📖 Reading document...')
      const text = await readFile(file)

      setMsg('🧠 Gemini AI analyzing... (auto-retrying if rate limited)')
      ...
      console.error(e)
      notify('❌ ' + (e.message || 'Failed — try again in a minute.'), true)
    }
    setLoading(false)
    setMsg('')
  }
  ...
  const startQuiz = async (sub) => {
    setLoading(true)
    try {
      setMsg('🎯 Generating adaptive quiz... (auto-retrying if rate limited)')
      ...
      console.error(e)
      notify('❌ ' + (e.message || 'Quiz failed — try again in a minute.'), true)
    }
    setLoading(false)
    setMsg('')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      ...
      {toast && <div style={{ position: 'fixed', top: 18, right: 18, zIndex: 9999, background: toast.err ? 'var(--err)' : 'var(--a3)', color: '#fff', padding: '12px 20px', borderRadius: 12, fontWeight: 600, boxShadow: '0 8px 32px #0008', animation: 'fadeUp 0.3s ease', fontSize: 14, maxWidth: 360 }}>{toast.m}</div>}

      {loading && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(7,9,15,0.93)', zIndex: 9998, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 18 }}>
          ...
          <p style={{ color: 'var(--muted)', fontSize: 12 }}>Auto-retrying if rate limited ⚡</p>
        </div>
      )}
      ...
      <nav style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '0 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 62, position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 13, color: 'var(--muted)' }}>{user.displayName?.split(' ')[0] || user.email?.split('@')[0]}</span>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      ...
      <div className="card float" style={{ padding: '48px 44px', maxWidth: 460, width: '90%', textAlign: 'center', position: 'relative' }}>
        ...
        {[['📄', 'PDF, Word & PowerPoint support'], ['🧠', 'Gemini 2.0 Flash AI quiz generation'], ['📈', 'Auto-scaling Easy → Expert difficulty'], ['🔁', 'Spaced repetition for weak topics'], ['📅', 'Smart weekly study timetable'], ['☁️', 'Cloud sync — data never lost']].map(([i, l]) => (
        ...
        {error && <div style={{ padding: '10px 14px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, color: 'var(--err)', fontSize: 13, marginBottom: 12 }}>❌ {error}</div>}

        <button className="btn" onClick={handleLogin} disabled={signingIn}
          style={{ marginTop: 16, width: '100%', padding: 15, background: '#fff', color: '#111', borderRadius: 12, fontSize: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 700 }}>
          {signingIn ? (
            <><div style={{ width: 20, height: 20, border: '2px solid #ccc', borderTop: '2px solid #333', borderRadius: '50%', animation: 'spin 0.85s linear infinite' }} /> Signing in...</>
          ) : (
        ...
        <p style={{ marginTop: 12, fontSize: 11, color: 'var(--muted)' }}>Powered by Gemini 2.0 Flash · Data saved to Firestore</p>
      </div>
    </div>
  )
}
