    ...
    <div className="fade">
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:14,marginBottom:28}}>
        {subs.length===0?(
          ...
          <div key={s.id} className="card" style={{padding:22,borderLeft:`3px solid ${s.color}`,transition:'all 0.2s'}} onMouseOver={e=>e.currentTarget.style.transform='translateY(-2px)'} onMouseOut={e=>e.currentTarget.style.transform='translateY(0)'}>
            ...
            <div style={{display:'flex',gap:14,marginBottom:14,fontSize:12,color:'var(--muted)'}}>
              <span>📝 {s.sessions||0} sessions</span><span>⭐ {s.avgScore||0}%</span><span>🗓 {s.lastStudied||'Never'}</span>
            </div>
      ...
      {wData.some(d=>d.min>0)&&(
        <div className="card" style={{padding:24}}>
          ...
          <ResponsiveContainer width="100%" height={150}>
            <BarChart data={wData}><XAxis dataKey="day" tick={{fill:'var(--muted)',fontSize:11}} axisLine={false} tickLine={false}/><YAxis hide/><Tooltip contentStyle={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,fontSize:12}}/><Bar dataKey="min" fill="var(--accent)" radius={[6,6,0,0]} name="Minutes"/></BarChart>
          </ResponsiveContainer>
        </div>
      )}
    </div>
...
function Upload({onUpload}) {
  const [file,setFile]=useState(null); const [name,setName]=useState(''); const [drag,setDrag]=useState(false); const ref=useRef()
  ...
  return (
    <div className="fade" style={{maxWidth:560,margin:'0 auto'}}>
      ...
      <div className="card" style={{padding:30}}>
        <div onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);const f=e.dataTransfer.files[0];if(f)pick(f)}} onClick={()=>ref.current?.click()}
...
function Quiz({sub,state,setState,onFinish,onBack}) {
  ...
  const q=questions[current]; const ref=useRef()
  useEffect(()=>{
    if(done)return
    setState(s=>({...s,timeLeft:30}))
    ref.current=setInterval(()=>{setState(s=>{if(s.timeLeft<=1){clearInterval(ref.current);answer(-1,s);return s}return{...s,timeLeft:s.timeLeft-1}})},1000)
  ...
  if(done){
    const pct=finalScore||0; const col=pct>=70?'var(--a3)':'var(--warn)'
  ...
  return(
    <div className="fade" style={{maxWidth:640,margin:'0 auto'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:22}}>
        ...
        <div style={{textAlign:'center'}}><div style={{fontWeight:700,fontSize:14}}>{sub.name}</div><div style={{fontSize:12,color:'var(--muted)'}}>Q {current+1} / {questions.length}</div></div>
        <div style={{width:44,height:44,borderRadius:'50%',background:state.timeLeft<=10?'rgba(239,68,68,0.15)':'rgba(0,200,255,0.1)',border:`2px solid ${state.timeLeft<=10?'var(--err)':'var(--accent)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,color:state.timeLeft<=10?'var(--err)':'var(--accent)',fontSize:16}}>{state.timeLeft}</div>
      </div>
      ...
      <div className="card" style={{padding:26,marginBottom:16}}><p style={{fontSize:17,fontWeight:600,lineHeight:1.65}}>{q.q}</p></div>
      <div style={{display:'flex',flexDirection:'column',gap:10,marginBottom:16}}>
        {q.options?.map((opt,i)=>(
          <button key={i} className="btn" onClick={()=>answer(i)}
            ...
            <span style={{width:28,height:28,borderRadius:'50%',background:'var(--border)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,flexShrink:0}}>{['A','B','C','D'][i]}</span>{opt}
          </button>
        ))}
      </div>
      <div style={{display:'flex',gap:10}}>
        <button className="btn" onClick={()=>setState(s=>({...s,showHint:!s.showHint}))} style={{flex:1,padding:9,background:'rgba(245,158,11,0.1)',color:'var(--warn)',border:'1px solid rgba(245,158,11,0.25)',fontSize:13}}>💡 {showHint?'Hide':'Show'} Hint</button>
        <button className="btn" onClick={()=>answer(-1)} style={{flex:1,padding:9,background:'var(--surface)',color:'var(--muted)',border:'1px solid var(--border)',fontSize:13}}>⏭ Skip</button>
      </div>
      {showHint&&q.hint&&<div style={{marginTop:14,padding:15,background:'rgba(245,158,11,0.06)',borderRadius:12,border:'1px solid rgba(245,158,11,0.2)',fontSize:14,color:'var(--warn)',lineHeight:1.6}}>💡 <strong>Hint:</strong> {q.hint}</div>}
    </div>
  )
}
...
function Schedule({subs}) {
  ...
  const sched=DAYS.map((day,di)=>({day,slots:subs.filter((_,si)=>(si+di)%Math.max(Math.ceil(subs.length/2),1)===0||subs.length===1).slice(0,2)}))
  return(
    <div className="fade">
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:10,marginBottom:28}}>
        {sched.map(({day,slots})=>(
          <div key={day} className="card" style={{padding:14,minHeight:170}}>
            ...
            {slots.length===0?<div style={{textAlign:'center',color:'var(--muted)',fontSize:11,marginTop:28}}>Rest 😴</div>:slots.map(s=>(
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:12}}>
          {subs.map(s=>(
            <div key={s.id} style={{padding:15,background:'var(--surface)',borderRadius:12,border:'1px solid var(--border)'}}>
              ...
              <div style={{fontSize:12,color:'var(--muted)'}}>Review every: <strong style={{color:s.avgScore>=80?'var(--a3)':'var(--warn)'}}>{s.avgScore>=80?'7 days':s.avgScore>=60?'3 days':'Daily'}</strong></div>
...
function Progress({subs}) {
  ...
  const wData   = DAYS.map((d,i)=>({day:d,min:subs.reduce((a,s)=>(s.sessions||0)>i?a+35:a,0)}))
  const history = Array.from({length:5},(_,i)=>{const obj={session:`S${i+1}`};subs.forEach(s=>{obj[s.name.slice(0,8)]=s.scores?.[i]??null});return obj})
  return(
    <div className="fade">
        ...
        <div className="card" style={{padding:24}}>
          ...
          <ResponsiveContainer width="100%" height={210}>
            <BarChart data={pData} margin={{left:-20}}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/><XAxis dataKey="name" tick={{fill:'var(--muted)',fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:'var(--muted)',fontSize:11}} domain={[0,100]}/><Tooltip contentStyle={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,fontSize:12}}/><Bar dataKey="score" fill="var(--accent)" radius={[6,6,0,0]} name="Score %"/>
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="card" style={{padding:24}}>
          ...
          <ResponsiveContainer width="100%" height={210}>
            <LineChart data={wData} margin={{left:-20}}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/><XAxis dataKey="day" tick={{fill:'var(--muted)',fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:'var(--muted)',fontSize:11}}/><Tooltip contentStyle={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,fontSize:12}}/><Line type="monotone" dataKey="min" stroke="var(--a2)" strokeWidth={2.5} dot={{fill:'var(--a2)',r:4}} name="Minutes"/>
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="card" style={{padding:24,marginBottom:20}}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={history} margin={{left:-20}}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/><XAxis dataKey="session" tick={{fill:'var(--muted)',fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:'var(--muted)',fontSize:11}} domain={[0,100]}/><Tooltip contentStyle={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:8,fontSize:12}}/><Legend wrapperStyle={{fontSize:12,color:'var(--muted)'}}/>
      ...
      <div className="card" style={{padding:24}}>
        ...
        <table>
          <thead><tr style={{borderBottom:'1px solid var(--border)'}}>
            {['Subject','Sessions','Avg Score','Best','Difficulty','Trend'].map(h=><th key={h} style={{textAlign:'left',padding:'9px 12px',color:'var(--muted)',fontWeight:700,fontSize:12}}>{h}</th>)}
          </tr></thead>
          <tbody>
            {subs.map(s=>(
              <tr key={s.id} style={{borderBottom:'1px solid rgba(28,42,64,0.6)'}}>
                <td style={{padding:'11px 12px',fontWeight:600}}><span style={{display:'inline-block',width:8,height:8,borderRadius:'50%',background:s.color,marginRight:10}}/>{s.name}</td>
                <td style={{padding:'11px 12px',color:'var(--muted)'}}>{s.sessions||0}</td>
                <td style={{padding:'11px 12px'}}><span style={{color:s.avgScore>=70?'var(--a3)':s.avgScore>=50?'var(--warn)':'var(--err)',fontWeight:700}}>{s.avgScore||0}%</span></td>
                <td style={{padding:'11px 12px',color:'var(--muted)'}}>{s.scores?.length?Math.max(...s.scores)+'%':'—'}</td>
                <td style={{padding:'11px 12px'}}><span style={{padding:'3px 9px',background:`${DCOL[s.difficulty]}18`,color:DCOL[s.difficulty],borderRadius:20,fontSize:11,fontWeight:700}}>{DIFFS[s.difficulty]}</span></td>
                <td style={{padding:'11px 12px',color:'var(--muted)',fontSize:13}}>{(s.scores?.length||0)>1?(s.scores.at(-1)>s.scores.at(-2)?'📈 Improving':'📉 Needs work'):'—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
